...
int main(int argc, char** argv) {
    ...
    // Create window
    GLFWwindow* window = glfwCreateWindow(windowW, windowH, "Fluid Simulation | ID: 111111111", NULL, NULL);
    if (!window) 
    {
        glfwTerminate();
        return -1;
    ...
    while (!glfwWindowShouldClose(window)) 
    {
        ...
        ImGui::Checkbox("Enable Background Cycle", &bgColorCycle);
        ImGui::End();
        if (bgColorCycle) {
            // Increment hue slowly
            hue += 0.001f;
